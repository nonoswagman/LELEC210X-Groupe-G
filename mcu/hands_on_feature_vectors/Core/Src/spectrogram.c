...
void Spectrogram_Format(q15_t *buf)
{
	...
	//            --> Pointwise shift
	//            Complexity: O(n) car on shift chaque élement
	...
	//            --> Pointwise substract
	//            Complexity: O(n) car on retire a chaque valeur 2**14
	//            Number of cycles: <TODO>
	...
	// [?] Why don't we use the full scale of the 16-bit? car necessite un bit de signe donc on est sur 15 bit, et pas le 16 eme pour eviter overflow
	// Are we done computing things with this array ? oui
...
//Le but de passer en fixed point est que les calculs sont bcp plus simple que en floating ou on doit jouer avec les exposants(meme si plus precis)
...
void Spectrogram_Compute(q15_t *samples, q15_t *melvec)
{
	...
	//           --> Pointwise product
	//           Complexity: O(n) car chaque element i de array 1 est multiplié par elem i de array 2
	//           Number of cycles: <TODO>
	start_cycle_count();
	...
	//           Complexity: O(n log n) cf internet
	//           Number of cycles: <TODO>
	...
	//           Complexity: O(n) car parcours la liste et updtate val_max
	//           Number of cycles: <TODO>
	start_cycle_count();
	q15_t vmax;
	uint32_t pIndex=0;
